...
inline auto HOTRowexChildPointer::castToNode(HOTRowexNodeBase const *node) {
  using DiscriminativeBitsRepresentationType =
      typename hot::commons::NodeTypeToNodeParameters<
          nodeAlgorithmType>::PartialKeyMappingType;
  using PartialKeyType = typename hot::commons::NodeTypeToNodeParameters<
      nodeAlgorithmType>::PartialKeyType;
  return reinterpret_cast<HOTRowexNode<DiscriminativeBitsRepresentationType,
                                       PartialKeyType> const *>(node);
}

template <hot::commons::NodeType nodeAlgorithmType>
inline auto HOTRowexChildPointer::castToNode(HOTRowexNodeBase *node) {
  using DiscriminativeBitsRepresentationType =
      typename hot::commons::NodeTypeToNodeParameters<
          nodeAlgorithmType>::PartialKeyMappingType;
  using PartialKeyType = typename hot::commons::NodeTypeToNodeParameters<
  ...
  return reinterpret_cast<
      HOTRowexNode<DiscriminativeBitsRepresentationType, PartialKeyType> *>(
      node);
}

template <typename Operation>
inline auto HOTRowexChildPointer::executeForSpecificNodeType(
    bool const withPrefetch, Operation const &operation) const {
  ...
  case hot::commons::NodeType::SINGLE_MASK_8_BIT_PARTIAL_KEYS:
    return operation(
        *castToNode<hot::commons::NodeType::SINGLE_MASK_8_BIT_PARTIAL_KEYS>(
            node));
  case hot::commons::NodeType::SINGLE_MASK_16_BIT_PARTIAL_KEYS:
    return operation(
        *castToNode<hot::commons::NodeType::SINGLE_MASK_16_BIT_PARTIAL_KEYS>(
            node));
  case hot::commons::NodeType::SINGLE_MASK_32_BIT_PARTIAL_KEYS:
    return operation(
        *castToNode<hot::commons::NodeType::SINGLE_MASK_32_BIT_PARTIAL_KEYS>(
            node));
  case hot::commons::NodeType::MULTI_MASK_8_BYTES_AND_8_BIT_PARTIAL_KEYS:
    return operation(
        *castToNode<
            hot::commons::NodeType::MULTI_MASK_8_BYTES_AND_8_BIT_PARTIAL_KEYS>(
            node));
  case hot::commons::NodeType::MULTI_MASK_8_BYTES_AND_16_BIT_PARTIAL_KEYS:
    return operation(
        *castToNode<
            hot::commons::NodeType::MULTI_MASK_8_BYTES_AND_16_BIT_PARTIAL_KEYS>(
            node));
  case hot::commons::NodeType::MULTI_MASK_8_BYTES_AND_32_BIT_PARTIAL_KEYS:
    return operation(
        *castToNode<
            hot::commons::NodeType::MULTI_MASK_8_BYTES_AND_32_BIT_PARTIAL_KEYS>(
            node));
  case hot::commons::NodeType::MULTI_MASK_16_BYTES_AND_16_BIT_PARTIAL_KEYS:
    return operation(
        *castToNode<hot::commons::NodeType::
                        MULTI_MASK_16_BYTES_AND_16_BIT_PARTIAL_KEYS>(node));
  default
      : // hot::commons::NodeType::MULTI_MASK_32_BYTES_AND_32_BIT_PARTIAL_KEYS:
    return operation(
        *castToNode<hot::commons::NodeType::
                        MULTI_MASK_32_BYTES_AND_32_BIT_PARTIAL_KEYS>(node));
  }
}

template <typename Operation>
inline auto
HOTRowexChildPointer::executeForSpecificNodeType(bool const withPrefetch,
...
inline HOTRowexChildPointer::HOTRowexChildPointer()
    : mPointer(reinterpret_cast<intptr_t>(nullptr)) {}

inline HOTRowexChildPointer::HOTRowexChildPointer(
    HOTRowexChildPointer const &other)
    : mPointer(other.mPointer.load(read_memory_order)) {}

inline HOTRowexChildPointer::HOTRowexChildPointer(
    hot::commons::NodeType nodeAlgorithmType, HOTRowexNodeBase const *node)
    : mPointer((reinterpret_cast<intptr_t>(node) |
                static_cast<intptr_t>(nodeAlgorithmType))
               << 1) {}
...
HOTRowexChildPointer::operator=(const HOTRowexChildPointer &other) {
...
inline bool
HOTRowexChildPointer::operator==(HOTRowexChildPointer const &other) const {
  return (mPointer.load(read_memory_order) ==
          other.mPointer.load(read_memory_order));
}
...
inline void HOTRowexChildPointer::free() const {
  executeForSpecificNodeType(false,
                             [&](const auto &node) -> void { delete &node; });
}

constexpr intptr_t NODE_ALGORITH_TYPE_HELPER_EXTRACTION_MASK =
    NODE_ALGORITHM_TYPE_EXTRACTION_MASK << 1;
inline hot::commons::NodeType HOTRowexChildPointer::getNodeType() const {
  const unsigned int nodeAlgorithmCode =
      static_cast<unsigned int>(mPointer.load(read_memory_order) &
                                NODE_ALGORITH_TYPE_HELPER_EXTRACTION_MASK);
...
inline intptr_t HOTRowexChildPointer::getTid() const {
  ...
  // normally this is undefined behaviour lookup for intrinsic working only on
  // x86 cpus replace with instruction for arithmetic shift
...
inline bool HOTRowexChildPointer::isAValidNode() const {
  ...
  return isNode(currentNodeValue) &
         (currentNodeValue != reinterpret_cast<intptr_t>(nullptr));
}

inline bool HOTRowexChildPointer::isEmpty() const {
  constexpr intptr_t EMPTY_CHILD_POINTER = reinterpret_cast<intptr_t>(nullptr)
                                           << 1;
...
HOTRowexChildPointer::search(uint8_t const *const &keyBytes) const {
  return executeForSpecificNodeType(
      true, [&](const auto &node) { return node.search(keyBytes); });
}
...
inline HOTRowexChildPointer
HOTRowexChildPointer::getSmallestLeafValueInSubtree() const {
  return isLeaf() ? *this
                  : getNode()->getPointers()[0].getSmallestLeafValueInSubtree();
}

inline HOTRowexChildPointer
HOTRowexChildPointer::getLargestLeafValueInSubtree() const {
  return isLeaf() ? *this
                  : getNode()
                        ->getPointers()[getNode()->getNumberEntries() - 1]
                        .getLargestLeafValueInSubtree();
}

bool HOTRowexChildPointer::compareAndSwap(
    HOTRowexChildPointer const &expected,
    HOTRowexChildPointer const &newValue) {
  intptr_t expectedContainedValue =
      expected.mPointer.load(std::memory_order_relaxed);
  return mPointer.compare_exchange_strong(
      expectedContainedValue,
      newValue.mPointer.load(std::memory_order_relaxed));
}
