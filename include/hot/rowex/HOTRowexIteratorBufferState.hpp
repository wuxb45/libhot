...
template <typename KeyType> struct HotRowexIteratorBufferState {
  ...
  HotRowexIteratorBufferState(HOTRowexChildPointer *const &buffer)
      : mBuffer(buffer), mCurrentBufferIndex(0u), mCurrentBufferSize(0),
        mLastAccessedKey() {
    buffer[0] = HOTRowexIteratorEndToken::END_TOKEN;
  }

  HotRowexIteratorBufferState(
      HotRowexIteratorBufferState<KeyType> const &other) = default;
  HotRowexIteratorBufferState<KeyType> &
  operator=(HotRowexIteratorBufferState<KeyType> const &other) = default;
  ...
  void copy(HotRowexIteratorBufferState const &other) {
    ...
    std::memmove(mBuffer, other.mBuffer,
                 sizeof(HOTRowexChildPointer) * other.mCurrentBufferSize);
  }
  ...
  bool isEmpty() { return mCurrentBufferSize == 0; }

  HOTRowexChildPointer const &getCurrent() const {
    return mBuffer[mCurrentBufferIndex];
  }

  void advance() { ++mCurrentBufferIndex; }

  bool canAdvance() { return (mCurrentBufferIndex + 1) < mCurrentBufferSize; }
  ...
  void setLastAccessedKey(KeyType const &lastAccessedKey) {
    mLastAccessedKey = idx::contenthelpers::toFixSizedKey(
        idx::contenthelpers::toBigEndianByteOrder(lastAccessedKey));
  }
