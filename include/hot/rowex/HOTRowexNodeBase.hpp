...
constexpr uint32_t ALL_ENTRIES_USED_SIMD_COB_TRIE_INCREMENTAL =
    UINT32_MAX; // 32 1 bits

namespace hot {
namespace rowex {

HOTRowexNodeBase::HOTRowexNodeBase(
    uint16_t const height,
    hot::commons::NodeAllocationInformation const &nodeAllocationInformation)
    : mFirstChildPointer(reinterpret_cast<HOTRowexChildPointer *>(
          reinterpret_cast<char *>(this) +
          nodeAllocationInformation.mPointerOffset)),
      mUsedEntriesMask(nodeAllocationInformation.mEntriesMask), mHeight(height),
      mLock(), mIsObsolete(false) {}
...
inline void HOTRowexNodeBase::unlock() { mLock.unlock(); }

inline HOTRowexChildPointer const *
HOTRowexNodeBase::toResult(uint32_t const resultMask) const {
  return getPointers() + toResultIndex(resultMask);
}

inline HOTRowexChildPointer *
HOTRowexNodeBase::toResult(uint32_t const resultMask) {
...
inline typename HOTRowexNodeBase::const_iterator
HOTRowexNodeBase::begin() const {
  return getPointers();
}
