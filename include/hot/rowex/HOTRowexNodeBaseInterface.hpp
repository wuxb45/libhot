...
public:
  /**
   * the height of this node. The height of a node is defined by the height of
   * its subtree. A leaf node therefore has height 1, its parent height 2 and so
   * forth.
   */
...
protected:
   ...
   * To determine the actual results the resultMask is intersected with the used
   * entries mask and the index of the most significant bit corresponds to the
   * result index. Finally the child pointer positioned at this index is
   * returned as the result candidate
   *
   ...
   * Therefore the resultMask is first restricted to the actual used entries and
   * then the index corresponding to the most significant set bit is returned as
   * the index of the actual result candidate
   *
   * @param resultMask the mask having bits set for all potential result
   * candidates
...
public:
  ...
  /**
   * @return whether the number of entries in this node corresponds to the
   * maximum node fanout. For HOTRowex the maximum node fanout is 32.
   */
  inline bool isFull() const;

  /**
   * @return whether this node was already replaced by an insert/delete
   * operation and is therefore obsolete. Therefore it will be garbage collected
   * in the future, when no thread holds any reference to this node.
   */
   ...
   *
   * @return wheter the lock can be aquired. It returns false if the node is in
   * state obsolete
   */
  inline bool tryLock();

  /**
   * marks this node as obsolete. This implies that it will eventually be
   * garbage collected.
   */
  ...
  /**
   * @return the pointer to the first child pointer stored in this node. All
   * other child pointers are stored sequentially following the first child
   * pointer.
   */
  ...
  /**
   * @return an iterator pointing to the first element after the last entry
   * stored in this node
   */
  inline iterator end();
