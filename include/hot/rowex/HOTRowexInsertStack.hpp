...
struct HOTRowexInsertStack {
  ...
  EntryType *getRawStack() { return reinterpret_cast<EntryType *>(mRawStack); }

  // do not initialize, for performance Reasons
  HOTRowexInsertStack(HOTRowexChildPointer currentRoot,
                      HOTRowexChildPointer *rootPointer,
                      uint8_t const *newKeyBytes)
      : mLeafEntry(reinterpret_cast<EntryType *>(mRawStack)) {
    ...
    while (!currentPointer.isLeaf()) {
      ...
      currentPointerLocation = currentPointer.executeForSpecificNodeType(
          true, [&, this](auto &node) {
            return node.searchForInsert(mLeafEntry->mSearchResultForInsert,
                                        newKeyBytes);
          });
  ...
  bool isConsistent(EntryType *firstLockedEntry,
                    unsigned int numberLockedEntries) {
    bool isConsistent = true;
    // start at minus one to check that the considered entry in the
    // firstLockedEntry is still the same (not already replaced or leaf node
    // pushdown or similar)
   ...
   */
  inline unsigned int
  tryLock(HOTRowexChildPointer *mRoot,
          HOTRowexFirstInsertLevel<EntryType> const &insertLevel) {
    ...
    if (lockedSuccessfully & (!insertLevel.mIsLeafNodePushdown) &
        (insertStackEntry > getRawStack())) {
      ...
      while ((lockedSuccessfully & (parentStackEntry > getRawStack())) &&
             ((currentNode.getNode()->isFull()) &
              ((currentNode.getHeight() + 1) ==
               parentStackEntry->getChildPointer().getHeight()))) {
    ...
    if (!lockedSuccessfully ||
        !isConsistent(insertStackEntry, numberLockedEntries)) {
    ...
    assert([&]() -> bool {
      EntryType *lastLockedEntry =
          firstInsertStackEntry + numberLockedEntries - 1;
      return (numberLockedEntries == 0 || lastLockedEntry != getRawStack() ||
              *mRoot == lastLockedEntry->getChildPointer());
    }());

    return numberLockedEntries;
  }

  HOTRowexFirstInsertLevel<EntryType>
  determineInsertLevel(hot::commons::DiscriminativeBit const &mismatchingBit) {
    EntryType *nextInsertStackEntry = getRawStack() + 1u;

    // searches for the node to insert the new value into., typename
    // InsertStackEntry Be aware that this can result in a false positive.
    // Therefor in case only a single entry is affected and it has a child node
    // it must be inserted into the child node this is an alternative approach to
    // using getLeastSignificantDiscriminativeBitForEntry
    while ((mismatchingBit.mAbsoluteBitIndex >
            nextInsertStackEntry->mSearchResultForInsert
                .mMostSignificantBitIndex) &
           (nextInsertStackEntry < mLeafEntry)) {
      ++nextInsertStackEntry;
    }

    EntryType *possibleInsertStackEntry = nextInsertStackEntry - 1u;
    // this is ensured because mMostSignificantDiscriminativeBitIndex is set to
    // MAX_INT16 for the leaf entry
    assert(possibleInsertStackEntry < mLeafEntry);

    hot::commons::InsertInformation const &insertInformation =
        possibleInsertStackEntry->getInsertInformation(mismatchingBit);
    bool isSingleEntry =
        possibleInsertStackEntry->isSingleEntryAffected(insertInformation);

    bool isLeafEntry = (nextInsertStackEntry == mLeafEntry);
    bool isBoundaryNode = isSingleEntry & (!isLeafEntry);

    return (isBoundaryNode)
               // in this case the single entry is a boundary entry -> insert
               // the value into the child partition As the insert results in a
               // new partition root, no prefix bits are set and all entries in
               // the partition are affected
               ? HOTRowexFirstInsertLevel<
                     EntryType>{nextInsertStackEntry,
                                {0, 0,
                                 static_cast<uint32_t>(
                                     nextInsertStackEntry->getChildPointer()
                                         .getNode()
                                         ->getNumberEntries()),
                                 mismatchingBit},
                                false}
               : HOTRowexFirstInsertLevel<EntryType>{
                     possibleInsertStackEntry, insertInformation,
                     isSingleEntry & isLeafEntry &
                         (possibleInsertStackEntry->getChildPointer()
                              .getHeight() > 1)};
  }

  idx::contenthelpers::OptionalValue<hot::commons::DiscriminativeBit>
  getMismatchingBit(uint8_t const *newKeyBytes) const {
    ...
    return hot::commons::getMismatchingBit(
        existingKeyBytes, newKeyBytes,
        static_cast<uint16_t>(idx::contenthelpers::getMaxKeyLength<KeyType>()));
  }
};
...
KeyExtractor<ValueType>
    HOTRowexInsertStack<ValueType, KeyExtractor, InsertStackEntry>::extractKey;
