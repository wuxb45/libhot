...
public:
  ...
  ThreadSpecificEpochBasedReclamationInformation()
      : mFreeLists(), mLocalEpoch(NOT_IN_EPOCH),
        mPreviouslyAccessedEpoch(NOT_IN_EPOCH), mThreadWantsToAdvance(false) {}
  ...
  void scheduleForDeletion(HOTRowexChildPointer const &childPointer) {
    ...
    std::vector<HOTRowexChildPointer> &currentFreeList =
  ...
  void leave() { mLocalEpoch.store(NOT_IN_EPOCH, std::memory_order_release); }
...
std::atomic<size_t>
    ThreadSpecificEpochBasedReclamationInformation::mNumberFrees{0};
