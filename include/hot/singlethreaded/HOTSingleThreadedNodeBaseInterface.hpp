...
protected:
  inline static MemoryPool<uint64_t, MAXIMUM_NODE_SIZE_IN_LONGS> *
  getMemoryPool();
  ...
  toResult(uint32_t resultMask) const;
...
public:
  /**
   * @return the total number of allocations executed on the underlying memory
   * pool
   */
   ...
   * maximum node fanout. For HOTSingleThreaded the maximum node fanout is 32.
   */
