...
public:
  ...
  FreeListEntry(FreeListEntry *next)
      : mNext(next), mListSize(next->mListSize + 1) {}

  FreeListEntry *getNext() { return mNext; }

  size_t getListSize() { return mListSize; }
};

template <typename ElementType, size_t NUMBER_LISTS,
          size_t EVICTION_BEGIN_SIZE = 200,
          size_t EVICTION_END_SIZE = EVICTION_BEGIN_SIZE / 2>
class MemoryPool;

template <typename ElementType, size_t NUMBER_LISTS, size_t EVICTION_BEGIN_SIZE,
          size_t EVICTION_END_SIZE>
class MemoryPool {
  static constexpr size_t SIZE_BEFORE_EVICTION_BEGIN_SIZE =
      EVICTION_BEGIN_SIZE - 1u;
...
public:
  ...
  MemoryPool &
  operator=(MemoryPool<ElementType, NUMBER_LISTS> const &other) = delete;

  ~MemoryPool() {
    for (FreeListEntry **freeList = mFreeLists.begin();
         freeList != mFreeLists.end(); ++freeList) {
  ...
  void *alloc(size_t numberElements) {
    ...
    if (head->getListSize() == 0) {
      ++mNumberAllocations;
      int error = posix_memalign(&rawMemory, sizeof(ElementType),
                                 numberElements * sizeof(ElementType));
      if (error != 0) {
  ...
  size_t getNumberAllocations() const { return mNumberAllocations; }

  size_t getNumberFrees() const { return mNumberFrees; }
...
template <typename ElementType, size_t NUMBER_LISTS, size_t EVICTION_BEGIN_SIZE,
          size_t EVICTION_END_SIZE>
FreeListEntry MemoryPool<ElementType, NUMBER_LISTS, EVICTION_BEGIN_SIZE,
                         EVICTION_END_SIZE>::TERMINATING_ENTRY{};
