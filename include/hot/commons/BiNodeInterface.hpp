...
template <typename ChildPointerType> struct BiNode {
  ...
  inline BiNode(uint16_t const discriminativeBitIndex, uint16_t const height,
                ChildPointerType const &left, ChildPointerType const &right);
  inline static BiNode
  createFromExistingAndNewEntry(DiscriminativeBit const &discriminativeBit,
                                ChildPointerType const &existingNode,
                                ChildPointerType const &newEntry);
};
