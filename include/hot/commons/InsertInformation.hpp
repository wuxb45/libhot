...
/**
 * The insert information contains all information, which is necessary to
 * actually insert a new key. It consists of:
 *  + the discriminative bit, which discriminates the entries in the affected
 * subtree from the new entry
 * 	+ a partial key which contains all discriminative bits,
 * 	  which are shared by the entries in the affected subtree and the new
 * key to insert
...
struct InsertInformation {
  /**
   * a partial key containing all discriminative bits, which are shared by the
   * entries in the affected subtree and the new key.
   */
  ...
  InsertInformation(uint32_t const subtreePrefixPartialKey,
                    uint32_t const firstIndexInAffectedSubtree,
                    uint32_t const numberEntriesInAffectedSubtree,
                    DiscriminativeBit const &keyInformation)
      : mSubtreePrefixPartialKey(subtreePrefixPartialKey),
        mFirstIndexInAffectedSubtree(firstIndexInAffectedSubtree),
