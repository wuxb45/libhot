...
class NodeMergeInformation {
  ...
  bool mHasMergedMask;
  alignas(std::alignment_of<MultiMaskPartialKeyMapping<4>>::value)
      std::array<uint8_t, sizeof(MultiMaskPartialKeyMapping<4>)> mRawMergedMask;
...
public:
   ...
   * @param right
   */
  template <typename SourceDiscriminativeBitsRepresentation1,
            typename SourceDiscriminativeBitsRepresentation2>
  NodeMergeInformation(uint16_t rootBit,
                       SourceDiscriminativeBitsRepresentation1 const &left,
                       SourceDiscriminativeBitsRepresentation2 const &right)
      : mKeyWithOnlyBitForLeftSourceInformation(
            left.createIntermediateKeyWithOnlySignificantBitsSet()),
        mKeyWithOnlyBitForRightSourceInformation(
            right.createIntermediateKeyWithOnlySignificantBitsSet()),
        mHasMergedMask(false) {
    initializeMergedMask(rootBit, mKeyWithOnlyBitForLeftSourceInformation,
                         mKeyWithOnlyBitForRightSourceInformation);
  };
  ...
  executeWithMergedDiscriminativeBitsRepresentationAndFittingPartialKeyType(
    ...
    assert(isValid());
    MultiMaskPartialKeyMapping<4> const &temporaryExtractionMask =
        *reinterpret_cast<MultiMaskPartialKeyMapping<4> const *>(
            mRawMergedMask.data());

    decltype(operation(temporaryExtractionMask, static_cast<uint32_t>(0u),
                       static_cast<uint32_t>(0u),
                       static_cast<uint32_t>(0u))) result;
    temporaryExtractionMask
        .executeWithCompressedDiscriminativeBitsRepresentation(
            [&](auto const &extractionInformation) {
                  ...
                  .executeWithCorrectMaskAndDiscriminativeBitsRepresentation(
                      [&](auto const &mergedDiscriminativeBitsRepresentation,
                        ...
                        uint32_t leftRecodingMask =
                            mergedDiscriminativeBitsRepresentation.extractMask(
                                mKeyWithOnlyBitForLeftSourceInformation.data());
                        ...
                        result = operation(
                            mergedDiscriminativeBitsRepresentation, maximumMask,
                            leftRecodingMask, rightRecodingMask);
                      });
            });
    return result;
  }

  bool isValid() const { return mHasMergedMask; }

private:
  void initializeMergedMask(uint16_t rootBit,
                            std::array<uint8_t, 256> const &first,
                            std::array<uint8_t, 256> const &second) {
    // AVX-512 ?
    ...
    uint8_t *rawBytePosition =
        reinterpret_cast<uint8_t *>(bytePositions.data());
    ...
    for (size_t i = 0; i < 256; i += sizeof(__m256i)) {
      __m256i firstPortion =
          _mm256_loadu_si256((__m256i const *)(first.data() + i));
      __m256i secondPortion =
          _mm256_loadu_si256((__m256i const *)(second.data() + i));
      __m256i bothBitsSet = _mm256_or_si256(firstPortion, secondPortion);

      BitMask32 usedByteIndexes(
          ~(_mm256_movemask_epi8(_mm256_cmpeq_epi8(zero, bothBitsSet))));

      _mm256_storeu_si256(reinterpret_cast<__m256i *>(result.data() + i),
                          _mm256_or_si256(firstPortion, secondPortion));
      ...
      for (uint32_t relativeUsedBytesIndex : usedByteIndexes) {
        ...
        if (nextBytePositionToUse == 0) {
          ...
          uint8_t rootByteMask =
              DiscriminativeBit(rootBit, 1).getExtractionByte();
    ...
    if (numberDiscriminativeBits <= 32) {
      ...
      new (mRawMergedMask.data()) MultiMaskPartialKeyMapping<4>(
          nextBytePositionToUse, numberDiscriminativeBits, bytePositions,
          byteMasks);
    } else {
      assert(false);
  ...
  static size_t popcount(std::array<uint64_t, 4> const &rawByteMasks) {
    return _mm_popcnt_u64(rawByteMasks[0]) + _mm_popcnt_u64(rawByteMasks[1]) +
           _mm_popcnt_u64(rawByteMasks[2]) + _mm_popcnt_u64(rawByteMasks[3]);
  }
};
