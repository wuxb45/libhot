...
/**
 * Describes a range of entries by its start index and the number of entries
 * contained.
 */
struct EntriesRange {
  ...
  EntriesRange(uint32_t firstIndexInRange, uint32_t numberEntriesInRange)
      : mFirstIndexInRange(firstIndexInRange),
        mNumberEntriesInRange(numberEntriesInRange) {}
