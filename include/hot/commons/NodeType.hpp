...
inline NodeType getRandomNodeType(uint numberKeyBits, uint numberRandomBytes) {
  ...
  // log2 + shift right ---> numberBytes 0-7 > 0, numberBytes 7 - 16 > 1,
  // numberBytes 16 - 32 > 2
  uint32_t numberExtractionMasks =
      (32 - __builtin_clz((numberRandomBytes - 1) / 8));

  uint moreThanASingleExtractionMask = (numberExtractionMasks > 0);

  // because random masks start at 3 + numberMaskBytes (counted from 0) see
  // commend in first line of this function
  // + in case the numberExtractionMasks is larger than 8 the number of mask
  // bits determine the number of of random bytes Hence adding if the mask size
  // is 16 (4) adding 2 will result in 6 which is for
  // MULTI_MASK_16_BYTES_AND_16_BIT_PARTIAL_KEYS. This mus be correct as the
  // number of random bytes can never be larger than the number of key bits
  // Therefore in the case of mask size 32 (5) adding 2 will result in 7 which
  // will be correct in cases where the number of extraction bytes is below 16
  // as well as above 16 because in both cases the only matching extraction mask
  // is MULTI_MASK_32_BYTES_AND_32_BIT_PARTIAL_KEYS
  return static_cast<NodeType>(3 + numberMaskBytes +
                               (moreThanASingleExtractionMask * 2));
}
