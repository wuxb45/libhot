...
inline NodeTemplate<SingleMaskPartialKeyMapping, uint8_t> *
createTwoEntriesNode(BiNode<ChildPointerType> const &binaryNode) {
  constexpr uint16_t NUMBER_ENTRIES_IN_TWO_ENTRIES_NODE = 2u;
  NodeTemplate<SingleMaskPartialKeyMapping, uint8_t> *node =
      new (NUMBER_ENTRIES_IN_TWO_ENTRIES_NODE)
          NodeTemplate<SingleMaskPartialKeyMapping, uint8_t>(
              binaryNode.mHeight, NUMBER_ENTRIES_IN_TWO_ENTRIES_NODE,
              SingleMaskPartialKeyMapping{
                  DiscriminativeBit{binaryNode.mDiscriminativeBitIndex}});
