...
std::vector<uint64_t> createRandomDataSet(size_t size, bool isVerbose = false) {
  ...
  return std::vector<uint64_t>(uniqueRandomValues.begin(),
                               uniqueRandomValues.end());
};

std::vector<uint64_t> creatDataSet(CommandParser const &params,
                                   std::string const &typeParamName) {
