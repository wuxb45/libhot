...
template <typename T> T lexical_cast(std::string argumentToCast) {
  T var;
...
public:
  CommandParser(int argc, char **argv,
                std::function<void()> usageMessageGenerator)
      : mArgc(argc), mArgv(argv), mArguments(),
        mUsageMessageGenerator(usageMessageGenerator) {
    parse();
  }
  ...
  void checkAllowedParams(std::set<std::string> const &allowedParams) const {
    for (std::pair<std::string, std::string> const &argument : mArguments) {
      if (allowedParams.find(argument.first) == allowedParams.end()) {
        if (isVerbose()) {
          std::cerr << "Parameter " << argument.first << "is not supported"
                    << std::endl;
        }
        printUsageMessage();
      }
  ...
  ParamType get(std::string const &paramName,
                ParamType const &defaultValue) const {
    bool hasParam = has(paramName);
    return hasParam ? lexical_cast<ParamType>(mArguments.at(paramName))
                    : defaultValue;
  }

  template <class ParamType>
  ParamType expect(std::string const &paramName) const {
    if (!has(paramName)) {
      if (isVerbose()) {
  ...
  template <class ParamType>
  std::vector<ParamType> getListOf(std::string const &paramName,
                                   std::vector<ParamType> allowedValues) const {
  ...
  template <class ParamType>
  ParamType expectOneOf(std::string const &paramName,
                        std::vector<ParamType> allowedValues) const {
    ParamType paramValue = expect<ParamType>(paramName);
    if (std::find(allowedValues.begin(), allowedValues.end(), paramValue) ==
        allowedValues.end()) {
      if (isVerbose()) {
  ...
  std::string expectExistingFile(std::string const &paramName) const {
    ...
    if (!fileExists(fileName)) {
      if (isVerbose()) {
        std::cerr << "File: " << fileName << " for param " << paramName
  ...
  bool isVerbose() const { return get<bool>("verbose", false); }
