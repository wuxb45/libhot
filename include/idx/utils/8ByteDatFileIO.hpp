...
inline std::vector<uint64_t> readDatFile(std::string const &fileName,
...
inline void readDatFile(std::string const &fileName, uint64_t *values,
                        size_t numberValues) {
  size_t recordsToRead;
  std::ifstream input{fileName};
  input >> recordsToRead;
  if (numberValues < recordsToRead) {
    std::cout << "Should read " << numberValues << " values but " << fileName
              << " only contains " << recordsToRead << " values." << std::endl;
    exit(1);
  }
...
inline void writeDataFile(std::ostream &output,
...
inline void writeDatFile(std::string const &fileName,
                         std::vector<uint64_t> const &values) {
