...
template <typename T, typename SwitchToMemberFunction>
void writeToYAMLObject(std::map<std::string, T> map, std::ostream &output,
                       const size_t depth, SwitchToMemberFunction theSwitcher) {
  // Stub which will never be called
}

template <typename T>
void writeToYAMLObject(std::map<std::string, T> map, std::ostream &output,
                       const size_t depth, SwitchingParam<false> theSwitcher) {
  std::string l0(depth * 2, ' ');

  for (std::pair<std::string, T> const &information : map) {
    output << l0 << information.first << ": " << information.second
           << std::endl;
  }
}
...
void writeToYAMLObject(std::map<std::string, T> map, std::ostream &output,
                       const size_t depth,
                       SwitchingParam<true> /* theSwitcher */) {
  std::string l0(depth * 2, ' ');
...
template <typename T, typename SwitchToFundamental>
void writeToYAMLGeneric(std::map<std::string, T> map, std::ostream &output,
                        const size_t depth,
                        SwitchToFundamental /* theSwitcher */) {
...
template <typename T>
void writeToYAML(std::map<std::string, T> map, std::ostream &output,
                 const size_t depth = 0) {
  writeToYAMLGeneric<T>(map, output, depth,
                        SwitchingParam<std::is_fundamental<T>::value>{});
}
...
struct OperationResult {
  ...
  OperationResult() { mExecutionState = false; }

#if defined(USE_PAPI) || defined(USE_COUNTERS)
  std::map<std::string, double> mCounterValues;

  OperationResult(bool executionState, int64_t durationInNanos,
                  uint64_t numberKeys, uint64_t numberOps)
      : mExecutionState(executionState), mDurationInNanos(durationInNanos),
        mNumberKeys(numberKeys), mNumberOps(numberOps), mCounterValues() {}

  OperationResult(bool executionState, int64_t durationInNanos,
                  uint64_t numberKeys, uint64_t numberOps,
                  std::map<std::string, double> const &counterValues)
      : mExecutionState(executionState), mDurationInNanos(durationInNanos),
        mNumberKeys(numberKeys), mNumberOps(numberOps),
        mCounterValues(counterValues) {}
#else
  OperationResult(bool executionState, uint64_t durationInNanos,
                  uint64_t numberKeys, uint64_t numberOps)
      : mExecutionState(executionState), mDurationInNanos(durationInNanos),
        mNumberKeys(numberKeys), mNumberOps(numberOps) {}
#endif
  ...
  void writeYAML(std::ostream &output, const size_t depth) const {
    ...
    for (std::pair<std::string, double> const &counterValue : mCounterValues) {
      output << l0 << counterValue.first << ": " << counterValue.second
             << std::endl;
    }
...
#if defined(USE_PAPI)
    if ((mCounterValues.find("numberInstructions") != mCounterValues.end()) &&
        (mCounterValues.find("numberCycles") != mCounterValues.end())) {
      output << l0 << "ipc: "
             << (mCounterValues.at("numberInstructions") /
                 static_cast<double>(mCounterValues.at("numberCycles")))
             << std::endl;
    }
#endif

    output << l0 << "millionOpsPerSecond: " << getMillionOpsPerSecond()
           << std::endl;
  }
};
 ...
 * The total amount of memory required by the index structure is mandatory, all
 * other collected statistics are purely optional and dependent on the actual
 * index structure
 */
...
public:
  std::map<std::string, OperationResult> &getResults() { return results; }

  std::map<std::string, OperationResult> getResults() const { return results; }
