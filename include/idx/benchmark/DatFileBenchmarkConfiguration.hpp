...
public:
  ...
  DatFileBenchmarkConfiguration(idx::utils::CommandParser const &params)
      : mInsertTuples(), mLookupTuples(),
        ...
        mDotFileLocation(
            mWriteDotFile ? params.expect<std::string>("writeDotRepresentation")
                          : "") {
    loadInsertTuples(params);
    loadLookupStrings(params);
  }

  DatFileBenchmarkConfiguration(
      DatFileBenchmarkConfiguration const &configuration)
      : mInsertTuples{configuration.mInsertTuples},
        mLookupTuples{configuration.mLookupTuples} {}

  bool isInsertOnly() { return mInsertOnly; }

  bool shouldWriteDotRepresentation() { return mWriteDotFile; }

  std::string getDotFileLocation() { return mDotFileLocation; }

  DatFile const &getInsertTuples() { return *mInsertTuples; }

  DatFile const &getLookupTuples() { return *mLookupTuples; }
...
private:
  ...
  void loadInsertTuples(idx::utils::CommandParser const &parser) {
    ...
    mInsertTuples =
        idx::benchmark::DatFile::read(inputFileName, parser.get("size", -1));

    if (parser.has("insertModifier")) {
      modifyDatFile(parser.expectOneOf<std::string>(
                        "insertModifier", {"sequential", "reverse", "random"}),
                    *mInsertTuples);
    }
  }
  ...
  void loadLookupStrings(idx::utils::CommandParser const &parser) {
    if (parser.has("lookupFile")) {
      ...
      mLookupTuples =
          idx::benchmark::DatFile::read(lookupFileName, parser.get("size", -1));
    } else if (!parser.has("lookupModifier")) {
      mLookupTuples = mInsertTuples;
    } else {
      mLookupTuples = std::shared_ptr<idx::benchmark::DatFile>(
          new idx::benchmark::DatFile(*mInsertTuples));
    };
    if (parser.has("lookupModifier")) {
      modifyDatFile(parser.expectOneOf<std::string>(
                        "lookupModifier", {"sequential", "reverse", "random"}),
                    *mLookupTuples);
    }
  }
};
