...
struct ConstCharStarComparator {
  bool operator()(std::pair<char *, size_t> s1,
                  std::pair<char *, size_t> s2) const {
...
public:
  ...
  StringBenchmarkConfiguration(idx::utils::CommandParser const &params)
      : mInsertStrings(new std::vector<std::pair<char *, size_t>>),
        ...
        mCommandParser(params), mRawArguments(params.getRawArguments()),
  ...
  StringBenchmarkConfiguration(
      StringBenchmarkConfiguration const &configuration) = default;
...
private:
  void readStrings(std::string const &fileName,
                   std::vector<std::pair<char *, size_t>> &resultVector,
                   size_t numberEntries = SIZE_MAX) {
    std::string line;
    std::ifstream input(fileName);
    while (std::getline(input, line) &&
           (numberEntries == SIZE_MAX || resultVector.size() < numberEntries)) {
      if (line.length() < 127) {
  ...
  void
  modifyStringVector(std::string modifierType,
                     std::vector<std::pair<char *, size_t>> &resultVector) {
    if (modifierType == "sequential") {
      ...
      std::sort(resultVector.begin(), resultVector.end(),
                ConstCharStarReverseComparator());
  ...
  void createInsertStrings(idx::utils::CommandParser const &parser) {
    ...
    if (parser.has("insertModifier")) {
      modifyStringVector(
          parser.expectOneOf<std::string>("insertModifier",
  ...
  void createLookupStrings(idx::utils::CommandParser const &parser) {
    ...
    if (parser.has("lookupModifier")) {
      modifyStringVector(
          parser.expectOneOf<std::string>("lookupModifier",
                                          {"sequential", "reverse", "random"}),
          *mLookupStrings);
    }
  }
