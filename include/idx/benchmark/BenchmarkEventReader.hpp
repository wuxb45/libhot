...
public:
  using CurentEventsTemporaryIndexType =
      idx::maphelpers::STLLikeIndex<std::map, ValueType, KeyExtractor>;

  BenchmarkEventReader(std::string const &eventsSourceFile)
      : mEventsSourceFile(eventsSourceFile) {}
  ...
  BenchmarkEventList readEvents(CurentEventsTemporaryIndexType &currentEvents) {
    ...
    processLines([&eventList, &currentEvents, this](std::string const &line,
                                                    size_t lineNumber) {
      BenchmarkBaseEvent *baseEvent =
          eventList.addEvent(this->readBenchmarkLine, line, lineNumber);
...
private:
  size_t estimateSize() {
    ...
    processLines(
        [&sizeEstimator, this](std::string const &line, size_t lineNumber) {
  ...
  void processLines(LineProcessor processLine) {
  ...
  inline void initializeEvent(
      ...
      idx::maphelpers::STLLikeIndex<std::map, ValueType, KeyExtractor>
          &currentEntries) {
    scanEvent.mLastResult = currentEntries.scan(scanEvent.mLookupKey,
