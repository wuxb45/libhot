...
public:
  DatFileBenchmark(int argc, char **argv, std::string const &dataStructureName)
      : mBinary(argv[0]), mBenchmarkable{},
        mBenchmarkConfiguration{
            DatFileBenchmarkCommandlineHelper(argc, argv, dataStructureName)
                .parseArguments()},
        mBenchmarkResults{}, mDataStructureName{dataStructureName} {}

  template <typename Function>
  void benchmarkOperation(std::string const &operationName,
                          DatFile const &tuples, int repeats,
  ...
  int run() {
    ...
    /*for(int i=0; i <
    mBenchmarkConfiguration.getLookupTuples().getNumberTuples(); ++i) {
            std::cout << "< " <<
    mBenchmarkConfiguration.getLookupTuples().getCell(i, 0) << "> "
                              << "< " <<
    ...
    if (!mBenchmarkConfiguration.isInsertOnly()) {
      ...
      benchmarkOperation(
          "lookup", lookupKeys, repeats,
          [&](DatFile const &tuples) { return mBenchmarkable.search(tuples); });
    }
    std::cout << "Lookup bytes:: "
              << mBenchmarkConfiguration.getLookupTuples().getTupleSizeInBytes()
              << std::endl;

    DatFile const &tuplesToIterate =
    ...
    benchmarkOperation(
        "iterate", tuplesToIterate, repeats,
        [&](DatFile const &tuples) { return mBenchmarkable.iterate(tuples); });
