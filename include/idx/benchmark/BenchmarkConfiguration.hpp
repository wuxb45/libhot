...
public:
  ...
  BenchmarkConfiguration(idx::utils::CommandParser const &params)
      : mInsertKeys(new std::vector<uint64_t>),
        mLookupKeys(new std::vector<uint64_t>), mCommandParser(params),
  ...
  std::vector<uint64_t> &getInsertValues() { return *mInsertKeys; }

  std::vector<uint64_t> &getLookupValues() { return *mLookupKeys; }
  ...
  void createLookupKeys(idx::utils::CommandParser const &parser) {
    if (parser.has("lookup")) {
      ...
      if (lookupType == "sequential") {
        *mLookupKeys =
            std::vector<uint64_t>(mInsertKeys->begin(), mInsertKeys->end());
        idx::utils::sortData(*mLookupKeys);
      } else if (lookupType == "reverse") {
        *mLookupKeys =
            std::vector<uint64_t>(mInsertKeys->rbegin(), mInsertKeys->rend());
