...
std::vector<std::string> readSpaceSeparatedTuple(std::string line) {
  ...
  return {std::istream_iterator<std::string>{lineInput},
...
private:
  ...
  DatFile(std::vector<uint> columnSizes, std::vector<uint> columnOffsets,
          uint tupleSizeInBytes, std::vector<uint8_t *> &&rawTuples)
      : mColumnSizes(columnSizes), mColumnOffsets(columnOffsets),
        mTupleSizeInBytes(tupleSizeInBytes), mRawTuples(rawTuples) {}

public:
  // RValue Copy Constructor needed...
  DatFile(DatFile &&rValue)
  ...
  DatFile(DatFile const &other)
      : mColumnSizes(other.mColumnSizes), mColumnOffsets(other.mColumnOffsets),
  ...
  uint getTupleSizeInBytes() const { return mTupleSizeInBytes; }

  uint64_t getNumberTuples() const { return mRawTuples.size(); }
  ...
  uint64_t getCell(size_t tupleId, size_t columnId) const {
    ...
    case 2:
      return __builtin_bswap16(
          *reinterpret_cast<uint16_t *>(rawTuple + mColumnOffsets[columnId]));
      break;
    case 4:
      return __builtin_bswap32(
          *reinterpret_cast<uint32_t *>(rawTuple + mColumnOffsets[columnId]));
      break;
    case 8:
      return __builtin_bswap64(
          *reinterpret_cast<uint64_t *>(rawTuple + mColumnOffsets[columnId]));
      break;
    default:
      std::cout << "Unsupported column size: " << mColumnSizes[columnId]
                << std::endl;
      exit(1);
      break;
    }
  ...
  uint8_t *getRawTuple(size_t tupleId) const { return mRawTuples[tupleId]; }

  void sort() {
    std::sort(mRawTuples.begin(), mRawTuples.end(),
              TupleComparator(mTupleSizeInBytes));
  }

  void reverseSort() {
    std::sort(mRawTuples.begin(), mRawTuples.end(),
              TupleReverseComparator(mTupleSizeInBytes));
  }
  ...
  void deduplicateSorted() {
    ...
    for (size_t currentTuple = 1ul; currentTuple < totalNumberTuples;
         ++currentTuple) {
      if (std::memcmp(getRawTuple(currentTuple),
                      getRawTuple(numberUnqiueTuples - 1),
                      mTupleSizeInBytes) != 0) {
        if (numberUnqiueTuples != currentTuple) {
          std::memcpy(getRawTuple(numberUnqiueTuples),
                      getRawTuple(currentTuple), mTupleSizeInBytes);
          ++numberUnqiueTuples;
        }
      }
    }
    mRawTuples.erase(mRawTuples.begin() + (numberUnqiueTuples - 1),
                     mRawTuples.end());
  }
...
private:
  ...
  static uint8_t *readRawTuple(std::istream &input,
                               std::vector<uint> columnSizes,
                               std::vector<uint> columnOffsets,
                               int bytesPerTuple) {
    uint8_t *rawTuple =
        new uint8_t[bytesPerTuple + 1]; // alignment for art tree maybe remove
    ...
    for (int columnIndex = 0; columnIndex < columnSizes.size(); ++columnIndex) {
      ...
      case 1:
        ...
        break;
      case 2:
        *reinterpret_cast<uint16_t *>(rawTuple + columnOffset) =
            __builtin_bswap16(columnValue);
        break;
      case 4:
        *reinterpret_cast<uint32_t *>(rawTuple + columnOffset) =
            __builtin_bswap32(columnValue);
        break;
      case 8:
        *reinterpret_cast<uint64_t *>(rawTuple + columnOffset) =
            __builtin_bswap64(columnValue);
        break;
      default:
...
public:
  static std::shared_ptr<DatFile> read(std::string const &datFileName,
                                       int64_t maxNumberEntries = -1) {
    ...
    size_t entriesToRead =
        maxNumberEntries < 0
            ? numberEntries
            : std::min((size_t)maxNumberEntries, numberEntries);
    rawTuples.reserve(entriesToRead);
    for (size_t i = 0; i < entriesToRead; ++i) {
      if ((i % 1000000) == 0) {
        std::cout << "Read " << i << " tuples of " << entriesToRead
                  << " tuples in total" << std::endl;
      }
      rawTuples.push_back(
          readRawTuple(input, columnSizes, columnOffsets, tupleSizeInBytes));
    }
    return std::shared_ptr<DatFile>(new DatFile(
        columnSizes, columnOffsets, tupleSizeInBytes, std::move(rawTuples)));
  }
};
