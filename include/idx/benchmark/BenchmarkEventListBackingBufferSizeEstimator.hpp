...
class BenchmarkEventListBackingBuferSizeEstimator {
  ...
  size_t totalBackingBufferSizeInBytes =
      (4096 / sizeof(max_align_t)) * sizeof(max_align_t);
...
public:
  BenchmarkEventListBackingBuferSizeEstimator()
      : mCountBuffer(backingBuffer.data(),
                     backingBuffer.size() *
                         sizeof(typename decltype(backingBuffer)::value_type)),
        mTotalBufferSize(0ul) {}

  template <typename EventConstructor, typename... Args>
  void addEvent(EventConstructor const &constructEvent, Args... args) {
    const std::pair<BenchmarkBaseEvent *, ContinuousBuffer> &event =
        constructEvent(args..., mCountBuffer);
    size_t eventSize =
        (mCountBuffer.mRemainingBufferSize - event.second.mRemainingBufferSize);
    mTotalBufferSize += eventSize;
    uintptr_t relativeAddress =
        (reinterpret_cast<char *>(event.second.mRemainingBuffer) -
         reinterpret_cast<char *>(backingBuffer.data()));
    size_t alignmentOffset = relativeAddress % sizeof(max_align_t);
    mCountBuffer = {reinterpret_cast<char *>(backingBuffer.data()) +
                        alignmentOffset,
                    totalBackingBufferSizeInBytes - alignmentOffset};
  }

  size_t getTotalBufferSize() { return mTotalBufferSize; }
};
