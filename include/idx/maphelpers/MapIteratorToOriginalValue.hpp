 ...
 * @tparam KeyExtractorType the key extractor used to extract the corresponding
 * key for given values
 */
...
struct MapIteratorToOriginalValue {
  ...
  using IteratorType =
      typename MapType<MapKeyType, MapValueType,
                       typename idx::contenthelpers::KeyComparator<
                           MapKeyType>::type>::const_iterator;
...
template <template <typename...> typename MapType, typename ValueType>
struct MapIteratorToOriginalValue<MapType, ValueType,
                                  idx::contenthelpers::IdentityKeyExtractor> {
  static MapValueExtractor<ValueType, idx::contenthelpers::IdentityKeyExtractor>
      toValue;
  using MapValueType = decltype(toValue(std::declval<ValueType>()));

  using IteratorType =
      typename MapType<ValueType, MapValueType,
                       typename idx::contenthelpers::KeyComparator<
                           ValueType>::type>::const_iterator;
...
/**
 * A template spezialization in case the stored value represents a pair and
 * therefore the iterator itself is able the represent the stored pair
 *
 * @tparam MapType the container type
 * @tparam ValueType the value type must be pair like, meaning that it has
 * fields first and second
 */
template <template <typename...> typename MapType, typename ValueType>
struct MapIteratorToOriginalValue<MapType, ValueType,
                                  idx::contenthelpers::PairKeyExtractor> {
  static idx::contenthelpers::PairKeyExtractor<ValueType> toKey;
  static MapValueExtractor<ValueType, idx::contenthelpers::PairKeyExtractor>
      toValue;
  ...
  auto operator()(IteratorType const &iterator) { return *iterator; }
};
