...
/**
 * returns a big endian representation of the given key. If the key is already
 * in big endian representation the key itself will be returned
 *
...
inline auto toBigEndianByteOrder(KeyType const &key) {
  return key;
};

template <>
__attribute__((always_inline)) inline auto
toBigEndianByteOrder<uint64_t>(uint64_t const &key) {
 ...
 */
template <typename KeyType>
constexpr inline __attribute__((always_inline)) size_t getMaxKeyLength() {
...
template <typename KeyType>
inline __attribute__((always_inline)) auto toFixSizedKey(KeyType const &key) {
  return key;
}

/**
 * return a fixed size key. A fixed sized key is a deterministic length
 * representation for a key. For instance for cstrings it is a 256 byte long key
 * representation
 *
 * @tparam KeyType the type of the key
...
template <> inline auto toFixSizedKey(char const *const &key) {
  ...
  strncpy(reinterpret_cast<char *>(fixedSizeKey.data()), key,
          getMaxKeyLength<char const *>());
  return fixedSizeKey;
}
 ...
 */
template <typename KeyType>
__attribute__((always_inline)) inline uint8_t const *
interpretAsByteArray(KeyType const &key) {
  return reinterpret_cast<uint8_t const *>(&key);
}

template <>
inline uint8_t const *
interpretAsByteArray<const char *>(const char *const &cStringKey) {
