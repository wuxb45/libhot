...
class HotSingleThreadedBenchmarkWrapper {
  using TrieType = hot::singlethreaded::HOTSingleThreaded<
...
public:
  ...
  bool search(std::pair<char *, size_t> const &lookupKey) {
    idx::contenthelpers::OptionalValue<const char *> result =
        mTrie.lookup(lookupKey.first);
  ...
  idx::benchmark::IndexStatistics getStatistics() {
    std::pair<size_t, std::map<std::string, double>> stats =
        mTrie.getStatistics();
    return {stats.first, stats.second};
  }

  void writeDotRepresentation(std::ostream & /* output */) { return; }
};

int main(int argc, char **argv) {
  idx::benchmark::StringBenchmark<HotSingleThreadedBenchmarkWrapper> benchmark(
      argc, argv, "HotSingleThreadedStringsBenchmark");
